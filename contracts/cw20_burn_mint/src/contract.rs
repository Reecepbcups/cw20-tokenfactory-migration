...
pub fn instantiate(
    ...
    let state = State {        
        cw20_address,
        tf_denom,
        prev_admin: deps.api.addr_validate(&msg.admin.unwrap_or(info.sender.to_string()))?,  
    };    
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Receive(cw20_msg) => {
            execute_redeem_mint(deps, info, cw20_msg)            
        },
        ExecuteMsg::TransferBackAdmin {  } => {
            execute_transfer_back_admin(deps, info)
        },
    }
}
...
pub fn execute_transfer_back_admin(
    ...
    let msg = TokenMsg::ChangeAdmin { denom: state.tf_denom, new_admin_address: state.prev_admin.to_string() };        
...
pub fn execute_redeem_mint(    
    ...
    let mint_tokens_msg = TokenMsg::mint_contract_tokens(state.tf_denom, cw20_msg.amount, cw20_msg.sender.clone());
